...
class Hipmob
{
  ...
  public function get_application($id, $verify = true)
  {
    if(!$verify){
      return new Hipmob_App($this, (object)array('id'=> $id, 'name'=> 'not set', 'url' => 'not set', 'created' => false));
    }
    $res = false;
  ...
  public function _send_text_messages($appid, $deviceids, $text)
  {
    $app = trim($appid);
    if(!$app || $app == "") throw new Hipmob_ApplicationNotSpecifiedError(400, "No application specified"); 
    if(!$deviceids || !is_array($deviceids) || count($deviceids) == 0) throw new Hipmob_DeviceNotSpecifiedError(400, "No device specified"); 
    if(!$text || trim($text) == "") throw new Hipmob_InvalidMessageContentError(400, "No message content specified");
    $res = false;
    ...
    // make the request
    $url = $this->baseurl . "apps/" . $app . "/devices/messages";
    $header = sprintf('Authorization: Basic %s', base64_encode($this->username.':'.$this->apikey));
    $header .= "\r\nContent-Type: application/x-www-form-urlencoded";
    ...
    foreach($deviceids as $deviceid){
      $content = $content . "&devices=".urlencode($deviceid);
    }
    $context = stream_context_create(array('http' => array(
							   // set HTTP method
							   'method' => 'POST',
	...
	return 1;
      ...
      $pattern = "/HTTP\/1\.1 200 (\d*) messages sent\./";
